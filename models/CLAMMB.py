...
class CLAMMB(CLAM_SB):
    """
    instance_eval=True
    """
    def __init__(self, input_size=1024, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=4,
        instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, instance_eval=True):
    ...
    def forward(self, **kwargs):
        ...
        M = torch.mm(A, h) 
        logits = torch.empty(1, self.n_classes).float().to(device) # [1,4]
